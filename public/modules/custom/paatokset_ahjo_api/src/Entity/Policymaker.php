...
class Policymaker extends Node implements AhjoEntityInterface {
  ...
  public function preSave(EntityStorageInterface $storage): void {
    ...
    // Create a slug if it doesn't exist. Newer override existing slugs
    // with autogenerated values, so users can override the field value.
    if ($this->get('slug')->isEmpty()) {
      $value = $this->get('field_ahjo_title')->getString() . '-' . $this->get('field_dm_org_name')->getString();
      ...
      // Slug has unique constraint. This assumes that one organization
      // level does not have two policymakers with the same name. Need
      // to revisit this assumption does not hold.
      $this->set('slug', $value);
    }
  }
